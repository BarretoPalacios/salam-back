...
def create_new_category(categoria: CategoryCreate, db: Session = Depends(get_db)):
...
def update_existing_product(category_id: int, category: CategoryUpdate, db: Session = Depends(get_db)):
    return update_category(db, category_id, category)

@router.delete("/{category_id}")
def delete_existing_product(category_id: int, db: Session = Depends(get_db)):
