...
def create_new_category(marca: MarcaCreate, db: Session = Depends(get_db)):
...
def update_existing_product(marca_id: int, marca: MarcaUpdate, db: Session = Depends(get_db)):
    return update_marca(db, marca_id, marca)

@router.delete("/{marca_id}")
def delete_existing_product(marca_id: int, db: Session = Depends(get_db)):
